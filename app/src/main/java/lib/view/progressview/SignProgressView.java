...
public class SignProgressView extends View {
    ...
    public SignProgressView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public SignProgressView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initValues() {
        circleRadius = WenziUtils.dip2px(getContext(), CIRCLE_RADIUS);
        stokenRadius = WenziUtils.dip2px(getContext(), STOKEN_RADIUS);
        stokenWidth = WenziUtils.dip2px(getContext(), STOKEN_WIDTH);
        linePadding = WenziUtils.dip2px(getContext(), LINE_PADDING);
        textPaddingTop = WenziUtils.dip2px(getContext(), TEXTPADDINGTOP);
        normaltextSize = WenziUtils.sp2px(getContext(), NORMALTEXTSIZE);
    }
    ...
    private boolean needStoken(int i) {
        ...
        Model model = mModels.get(i);
        if (model.state == SignProgressView.STARTING) {
            needStoken = true;
        }
